...
5-fold Repeated Cross-Validation (N=10)
...
Here I call the functions written above.
```{r}
...
Our data after remove rows with `NA` total to 395. Current guidance suggests a rule of thumb where there should be about 10 times the number of model parameters that are being estimated by the data (Harrell et al., 1996).  Models with high dimensionality are likely to over-fit i.e. the final model has large variance. Our final model had *REPLACE WITH FINAL NUMBER OF VARIABLES** parameters which is likely to lead to low bias but very high variance.

To get a more parsimonious model, we chose to use LASSO (L1) regression which is a type of regularisation method that penalises variables with minimal contribution to the outcome by shrinking their coefficient. We chose LASSO over Ridge regression because we wanted to reduce the model size and allow coefficients to be reduced to 0.
...
X = data %>% select(-fatalities)
model_matrix = model.matrix(~., X)
# Outcome fatalities
y = data %>% select(fatalities) %>% pull()
...
```
```{r}
anova(lm(fatalities ~ state + year + speed65 + age + income + drinkage + miles*seatbelt, df))
