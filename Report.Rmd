...
Here, lambda is the 'severity of punishment' when estimating coefficients and we use 10-fold cross-validation to select the lambda that produces the lowest out-of-sample MSE. The plot shows that the process still picked a model with 52 parametric estimates albeit with slightly different coefficents.
...
X = data_nona %>% dplyr::select(state, year, income, miles)
model_matrix = model.matrix(~ state + year + income + miles, X)
# Outcome fatalities
y = data_nona %>% dplyr::select(fatalities) %>% pull()
